...
public class ModelCubeWorld extends ModelBase {
    ...
    // Translate, rotate, translate back and then render.
    /**
     * sets the rotation point of the model centered if first three arguments
     * are 2 total size of the model gets devided by 16 to make 1x1 minecraft
     * pixels, then multiplied by the first three arguments to "center" the
     * rotation point angle is the degree of angle that needs to be rotated
     * *(180F / (float)Math.PI) (copied from modelbase to render softer
     * animations) RotatePoint. x / y / z to rotate around that point
     *
     * example : model.renderWithCenteredRotion(2, 2, 2, 20.0f, RotatePoint.Y);
     * */
    public void renderWithCenteredRotion(float xPosDevided, float yPosDevided, float zPosDevided, float angle, RotatePoint p) {

        float x = (getModelData().length / 16f) / xPosDevided;
        float y = (getModelData()[0].length / 16f) / yPosDevided;
        float z = (getModelData()[0][0].length / 16f) / zPosDevided;
        GL11.glTranslatef(x, y, -z);
        switch (p) {
            case X:
                GL11.glRotatef(angle * (180F / (float) Math.PI), 1, 0, 0);
                break;
            case Y:
                GL11.glRotatef(angle * (180F / (float) Math.PI), 0, 1, 0);
                break;
            case Z:
    ...
    /**
     * sets the rotation point of the model. x, y and zpos are the number of
     * .cub blocks to move the rotationpoint base rotation point starts in the
     * upperleft corner of the model angle is the degree of angle that needs to
     * be rotated *(180F / (float)Math.PI) (copied from modelbase to render
     * softer animations) RotatePoint. x / y / z to rotate around that point
     * */
    public void renderWithCenteredRotionPoint(float xpos, float ypos, float zpos, float angle, RotatePoint p) {
